        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title" style="font-size: 21px;">Company List</h3>

                    {{-- Data insert button with check permission --}}
                    <a href="{{ route('company.create') }}" class="btn btn-primary float-right nav-link">Add Company</a>
                </div>
...
@section('footer-script')
    ...
    <script>
        ...
        function deleteUser(id) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to delete this comapny",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    var url = "{{  route('company.index') }}"+"/"+ id; 
                    $.ajax({
                        type: 'DELETE',
                        url: url,
                        data: {"_token": "{{ csrf_token() }}"},
                        success: function () {
                            createMessage('Company deleted successfully')
                            table.ajax.reload();
                        },
                        beforeSend: function () {
                        },
