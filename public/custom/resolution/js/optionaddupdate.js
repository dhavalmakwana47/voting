$("#resolution-form").validate({
    ...
    submitHandler: function (form) {
        $is_valid = true;
        if (!$("#member-verify-btn").prop("disabled")) {
            alert("Please click to Upload&Verify button.");
            $is_valid = false;
        }
...
function addResolutionDetailsValidation() {
    $("[name^='resolution'][name$='[description]']").each(function () {
        $(this).rules("add", {
            required: true,
        });
    });

    $("[name^='resolution'][name$='[resolution_files]']").each(function () {
...
function add_option(count) {
    optionCount = $(".option-wrap").length;
    $("#option-div-" + count).append(`
            <div class="row option-wrap">
                <div class="form-group option col-6">
                    <label for="option1">Option Name</label>
                    <input type="text" name="options[${count}][${optionCount}][name]" class="form-control" required>
                </div>
                <div class="form-group option col-4">
                    <label for="option1">Image</label>
                    <input type="file" name="options[${count}][${optionCount}][image]" class="form-control">
                </div>
                <div class="form-group option col-2">
                    <button type="button" class="btn btn-danger required-section" onclick="$(this).closest('.row').remove()">Delete</button>
                </div>
            </div>
        `);
}
function AddResolutionDetalisRaw() {
    ...
    $.ajax({
        url: addMoreRoute, // Specify the route where you handle the file upload in your Laravel controller
        method: "POST",
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
            $("#resolution-files").append(response);
...
$("#resolution-files").on("click", ".option-delete-btn", function () {
    $(this).parent().parent().remove();
});
